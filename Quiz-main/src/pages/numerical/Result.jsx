...
const Result = ({ results, finalScore }) => {
  ...
  return (
    <div className='mx-4 my-4'>
      <Link to="/" className="bg-blue-500 text-white px-4 py-2 rounded ml-2 shadow-lg transform transition-transform duration-300 hover:scale-105 hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-300">Home</Link>

      <h1 className="text-center font-semibold text-3xl text-blue-600 my-4">Quiz Results</h1>
      <p className="text-center text-lg my-2">Final Score: {finalScore}</p>
      ...
      <button className="bg-blue-400 text-white py-1 px-2 rounded-md mt-4" onClick={() => setShowCorrectAnswers(!showCorrectAnswers)}>
        {showCorrectAnswers ? 'Hide Detailed Result' : 'Show Detailed Result'}
      </button>
      {showCorrectAnswers &&
        <div>
